...
# Regular grid
{
  c_f_vals = seq(10, 170, by = 20)
  k_g_vals = seq(100, 1700, by = 200)
  idx_cf = 1:length(c_f_vals)
  idx_kg = 1:length(k_g_vals)
  c_f_vals_core = c_f_vals
  k_g_vals_core = k_g_vals
}
# Fuller grid
if (FALSE) {
  c_f_vals = seq(5, 45+20, by = 20)
...
# sigma_cf = 0.07025 # for grid going up to 170 & 1700
t10 = 10
t4 = 4
q = 1
r = 0.1
...
# t=10 - aligned
c_f_sim = random_walk_gbm(n=(2*t10)*10000, mu=mu_cf, sigma=sigma_cf, t=2*t10, x0=median(c_f_vals_core))
...
# 3D Surface version
# PV Value function
out_dir = paste0(root,'/OneDrive - rff/Documents - RPE-Electric Power/OPEX CAPEX Price Risk/output/figures/matched_costs_large_grid/')
dir.create(out_dir)
setwd(out_dir)
...
scene_vf = list(xaxis=list(title='Green CAPEX<br>     ($M)'),
                ...
                zaxis=list(title='$B', range=c(0,20)),
...
scene_sd = list(xaxis=list(title='Green CAPEX<br>     ($M)'),
                yaxis=list(title='Fossil OPEX<br> ($M/year)'),
                zaxis=list(title='$M', range=c(0,3000)),
                camera=list(eye=list(x=1.25*-1*1.5, y=1.25*-1*1.5, z=1.25*0.75*1.5))) # default angles for x, y, and z are 1.25. Multiply by proportions to adjust
...
scene_sd_delta = list(xaxis=list(title='Green CAPEX<br>     ($M)'),
                      yaxis=list(title='Fossil OPEX<br> ($M/year)'),
                      zaxis=list(title='$M', range=c(-2000,1500)),
                      camera=list(eye=list(x=1.25*-1*1.5, y=1.25*-1*1.5, z=1.25*0.75*1.5))) # default angles for x, y, and z are 1.25. Multiply by proportions to adjust
