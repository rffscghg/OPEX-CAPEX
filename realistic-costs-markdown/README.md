...
want to tune our state space for OPEX and CAPEX costs to cover the range
of realistic values for actual investments, including fossil CAPEX and
green OPEX, which are constants in the current version of our model. We
are planning to present results for two scenarios: power plants and
cars.
...
will use the “Henry Hub Natural Gas Price” for fossil OPEX, although
because 2022 was a price spike, we will use the average of all of the
2023 monthly data currently available in the
[FRED](https://fred.stlouisfed.org/series/MHHNGSP) database. We also
plan to incorporate fixed O&M expenses in the CAPEX figures, and some
dimensional analysis is required to get to the correct units. The
unitless $\sigma$ and $\mu$ parameters do not require dimensional
analysis, so let’s start there:
