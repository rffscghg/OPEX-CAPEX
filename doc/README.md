...
data to portray a real-world example. We focused specifically on the
power plant example because of the availability of price data for
land-based wind and natural gas. Because our model runs in a state space
...
Also, the wind power data specify that they use 2022 dollars. I saw no
such information on the EIA page for Henry Hub prices, so it is
presumably present day (2023). Should we correct for inflation? It’s
just one year but consumer price inflation has been high in recent
years. Would we use the consumer price value for conversion (annual
average? November, since December is not published yet?), or would we
find a value specific to the power industry?
...
k_g_tidy <- k_g_raw %>%
    ...
    transmute(date = Date, onshore_wind_installed = parse_number(`2022 $/kW`)) %>%
    filter(!is.na(onshore_wind_installed)) %>%
    transmute(date, k_g = k_unit_conversion(onshore_wind_installed, onm_g_2023, CF_g, dr, T, q))

c_f_tidy <- c_f_raw %>%
    # Convert based on heat rate, as above
    transmute(date = Date, c_f = `Henry Hub Natural Gas Spot Price (Dollars per Million Btu)` * heat_rate + var_onm)

inner_join(k_g_tidy, c_f_tidy) %>%
