...
Hub natural gas data gives no year, so we assume that it needs to be
corrected for inflation. We do so with the [GDP price deflator data from
FRED](https://fred.stlouisfed.org/series/GDPDEF), scaling to 2022
dollars to match the wind power data.
...
# Combine Henry Hub and deflator data
c_f_def <- gdp_def %>% 
...
# Correct Henry Hub data
c_f_2022 <- c_f_def %>%
    transmute(date = Date, c_f_def = `Henry Hub Natural Gas Spot Price (Dollars per Million Btu)` * def_2022/deflate)
...
c_f_tidy <- c_f_2022 %>%
    ...
    transmute(date, c_f = c_f_def * heat_rate + var_onm)
